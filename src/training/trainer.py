...
class CNNTrainer:
    ...
    def train_fold(self, fold_idx: int, train_df, val_df, test_df) -> Dict[str, float]:
        ...
        print(
            f"Average Test Accuracy: {np.mean(list(test_metrics['accuracies'].values())):.4f}"
        )

        # Save model
        if getattr(self.config, "save_best_model", True):
            model_save_path = os.path.join(
                self.config.output_dir, "models", f"fold_{fold_idx+1}_best_model.pth"
            )
            torch.save(
                {
    ...
    def update_task_weights_dynamically(
        ...
        """
        if (
            hasattr(self.config, "dynamic_task_weighting")
            and self.config.dynamic_task_weighting
        ):
            ...
            for task in self.task_names:
                ...
                new_weights[task] = self.config.task_weights[task] * (
