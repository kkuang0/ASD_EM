...
class DDPTrainer:
    ...
    def _setup_loss_function(self):
        ...
        if (
            hasattr(self.config, "use_label_smoothing")
            and self.config.use_label_smoothing
        ):
    ...
    def create_dataloaders(
        ...
        train_dataset = self.dataset_cls(
            train_df,
            transform=get_em_transforms(
                image_size=self.image_size,
                is_training=True,
                apply_clahe_nlm=False,
            ),
        )
        ...
        if self.class_weights is None:
            ...
            if self.config.distributed:
                if self.config.rank == 0:
                    class_weights_data = {
                        task: w.cpu() for task, w in self.class_weights.items()
                    }
                else:
                ...
                if self.config.rank != 0:
                    ...
                    self.class_weights = {
                        task: w.to(self.device)
                        for task, w in class_weights_data.items()
                    }
    ...
    def train_epoch(
        ...
        for batch_idx, (images, labels) in enumerate(train_loader):
            ...
            labels = {
                k: v.to(self.device, non_blocking=True) for k, v in labels.items()
            }
            ...
            if (
            ...
            ) and batch_idx % getattr(self.config, "log_every_n_batches", 50) == 0:
                ...
                if writer:
                    ...
                    for task, task_loss in task_losses.items():
                        writer.add_scalar(
                            f"Loss/Train_{task}_Batch", task_loss.item(), step
                        )
                    for task in self.task_names:
                        batch_acc = (
                            (torch.argmax(outputs[task], dim=1) == labels[task])
                            .float()
                            .mean()
                        )
                        writer.add_scalar(
                            f"Accuracy/Train_{task}_Batch", batch_acc.item(), step
                        )

                print(
                    f"Rank {self.config.rank if self.config.distributed else 0} - "
        ...
        train_acc = {
            t: (correct[t] / total[t]) if total[t] > 0 else 0.0 for t in self.task_names
        }
        ...
        if self.config.distributed:
            for t in self.task_names:
                ...
                train_acc[t] = (
                    tensor[0].item() / tensor[1].item() if tensor[1].item() > 0 else 0.0
                )
    ...
    def _broadcast_dataframes(self, folds, test_df):
        ...
        if self.config.rank == 0:
            ...
            data_to_broadcast = {
                "folds": [
                    (train_df.to_dict(), val_df.to_dict()) for train_df, val_df in folds
                ],
        ...
        if self.config.rank != 0:
            ...
            folds = [
                (pd.DataFrame.from_dict(train_dict), pd.DataFrame.from_dict(val_dict))
    ...
    def train_fold(self, fold_idx: int, train_df, val_df, test_df) -> Dict[str, float]:
        ...
        for epoch in range(self.config.epochs):
            ...
            if self.config.distributed:
                stop_training = torch.tensor(
                    patience_counter >= max_patience, device=self.device
                )
            ...
            if not self.config.distributed or self.config.rank == 0:
                if writer:
                    ...
                    if val_eval_loader is not None:
                        detailed = self.evaluator.evaluate(
                            model, val_eval_loader, detailed=True
                        )
                ...
                print(
                    f"Average Val Accuracy: {avg_val_acc:.4f} - Best: {best_val_score:.4f}"
                )
        ...
        if not self.config.distributed or self.config.rank == 0:
            ...
            print(
                f"Average Test Accuracy: {np.mean(list(test_accuracies.values())):.4f}"
            )
    ...
    def train_cross_validation(self) -> Dict[str, List[float]]:
        ...
        if not self.config.distributed or self.config.rank == 0:
            ...
            if hasattr(self.config, "train_val_csv_path") and hasattr(
                ...
                raise ValueError(
                    "Config must have either 'csv_path' OR both 'train_val_csv_path' and 'test_csv_path'"
                )
        else:
